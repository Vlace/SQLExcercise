   ...
   SELECT app_name FROM analytics WHERE price BETWEEN 1 and .1 ORDER BY rating DESC LIMIT 10;
    ...
    SELECT category, count(category) FROM analytics GROUP BY category HAVING count(category) > 300  ORDER BY count(category);
...
SELECT app_name, rating, category FROM analytics WHERE (rating, category) IN (SELECT max(rating), category FROM analytics WHERE min_installs >= 50000 GROUP BY category);
